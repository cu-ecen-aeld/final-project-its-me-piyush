...
static const unsigned long US_CAN_TX_PERIOD_MS    = 500;
static const float         US_BLOCK_DISTANCE_CM   = 20.0f;
static const uint8_t       US_FILTER_SAMPLES      = 5;
...
bool isForwardCommand(int leftCmd, int rightCmd) {
  return (leftCmd > 0 && rightCmd > 0);
}

bool shouldBlockForward(int leftCmd, int rightCmd) {
  if (!ultrasonicValid) return false;
  if (!isForwardCommand(leftCmd, rightCmd)) return false;
  return ultrasonicCm > 0.0f && ultrasonicCm <= US_BLOCK_DISTANCE_CM;
}
...
void updateUltrasonic() {
  ...
  obstacleBlocked = ultrasonicValid && (ultrasonicCm <= US_BLOCK_DISTANCE_CM);

  if (debugMode) {
    if (ultrasonicValid) {
      Serial.printf("[US] distance=%.1f cm blocked=%s\n",
                    ultrasonicCm,
...
void printHelp() {
  ...
  Serial.println("sendus      : send ultrasonic CAN frame");
...
void loop() {
  ...
  if (canReady && piAlive) {
    ...
    if (now - lastUsTxMs >= US_CAN_TX_PERIOD_MS) {
      lastUsTxMs = now;
      sendUltrasonic();
    }
  }
